...
if(!empty($_SESSION["warehouse_cart"])){
		$pn = 1; $qn = 1; $an = 1;  foreach($_SESSION["warehouse_cart"] as $keys => $values)
		...
		{	
			...
			<tr id="p_'.$values["product_id"].'" class="'.$values["product_id"].'" data-item-id="'.$values["product_id"].'" data-id="'.$values["product_id"].'">
				<td><button type="button" class="btn bg-purple btn-block btn-xs edit" data-id="'.$values["product_id"].'"><span class="sname" id="name_'.$values["product_name"].'"><span class="sname" id="name_'.$values["product_name"].'">'.$values["product_name"] .' ('.$stockUnit.')</span></button> <input type="hidden" name="product_name[]" value="'.$values["product_name"].'" class="product_name'.$values["product_id"].'">
				...
				<td align="right"> '.$values["product_price"].' <input type="hidden" size="1" name="product_price[]" id="unit_cost'.$values["product_id"].'" value="'.$values["product_price"].'" class="unit_cost"></td>
				...
				<input type="hidden" size="1" class="tax" name="stockUnit[]" value="'.$stockUnit.'" id="stockUnit'.$values["product_id"].'">
