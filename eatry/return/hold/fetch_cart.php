...
if (!empty($_SESSION["return_cart"])) {
	...
	foreach ($_SESSION["return_cart"] as $keys => $values) {
		...
		$stockUnit = intval($values["stockUnit"]) - intval(intval($values["product_quantity"]));
		...
		$output .= '
			...
			<tr id="p_' . $values["product_id"] . '" class="' . $values["product_id"] . '" data-item-id="' . $values["product_id"] . '" data-id="' . $values["product_id"] . '">
				...
				<td align="right"> ' . intval($values["product_price"]) . ' <input type="hidden" name="product_price[]" id="unit_cost' . $values["product_id"] . '" value="' . intval($values["product_price"]) . '" class="unit_cost"></td>
				
				<td align="center">
				<input autocomplete="no" class="form-control input-qty kb-pad text-center qty" name="quantity[]" type="text" value="' . intval($values["product_quantity"]) . '" id="quantity' . $values["product_id"] . '" data-id="' . $values["product_id"] . '" max="' . $values["stockUnit"] . '">
				</td>
				
				<td align="right" id="total' . $values["product_id"] . '"> ' . number_format(intval($values["product_quantity"]) * intval($values["product_price"]), 2) . '
				<input type="hidden" size="1" class="amount" name="total_price[]" value="' . intval($values["product_quantity"]) * intval($values["product_price"]) . '" id="total_price' . $values["product_id"] . '">
				</td>
			...
			';
		$total_price = $total_price + (intval($values["product_quantity"]) * intval($values["product_price"]));
		$total_item = $total_item + 1;
		// $tax = $tax + (intval($values["product_quantity"]) * $values["product_tax"]);
		$tax = $tax + (intval($values["product_quantity"]) * 0);
		$sum += intval($values['product_quantity']);
	}
} else {
	$output .= '
	    <tr>
