                   ...
                   <h5 class="text-uppercase">Stock Record</h5>
                      <div class="d-flex justify-content-between pb-3">
                          <div class="btn-group d-none">
                            <a href="<?php echo url_for('stock/exportData.php') ?>" class="btn btn-success"><i class="fa fa-file-excel-o"></i> Export as CSV</a>
                          </div>
                           <div class="btn-group d-none">
                      ...
                      <table class="table table-sm table-bordered" id="example2">
                        ...
                        <tbody >
                             ...
                             foreach (Product::find_by_undeleted(['order' => 'ASC']) as $key => $item) {
                              ...
                              // pre_r($value);
                              $stock = StockDetails::sum_of_Stock(['from' => date('Y-m-d'), 'item_id' => $item->id]) ?? 0;
                              $sales = Sales::find_all_by_product_id(['product_id' => $item->id,'from'=>  date('Y-m-d') ]);
                              $qty = $sales ?? 0;
                              ...
                              ?>
                                <tr class="text-center">
                                  ...
                                  <td>
                                    <?php if($stock_mgt == 1){ ?>
                                          <a class="" href="<?php echo url_for('stock/items.php?id='.$item->id) ?>" style="text-decoration: underline;" data-id="<?php echo $item->id ?>">
                                            <?php echo $item->pname; ?>
                                          </a>
                                        <?php }else{ ?>
                                            <?php echo $item->pname; ?>
                                 ...
                                 <td><?php echo $left_over ?? 0; ?></td>
                                  ...
                                  <td>
                                    <button type="button" class=" btn btn-sm btn-primary add" data-id="<?php echo $item->id ?>"><i class="fa fa-plus"></i> Add Stock</button>
                                  </td>
                                </tr>
                              <?php } ?>
                        <tbody>
