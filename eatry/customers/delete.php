...
if (is_post_request()) {
  ...
  // logfile
  log_action('Delete customer', "id: {$customer->id}, Deleted by {$loggedInAdmin->full_name()}", "customer");

  // Delete customer
  $result = $customer->deleted($id);
  $session->message('The customer was deleted successfully.');
  redirect_to(url_for('/customers/index.php'));
} else {
  // Display form
}
          ...
          <div class="col-xs-12">
            <div class="card text-center ">
              <div class="bg-primary" style="padding: 10px">
                ...
                <p class="item"><?php echo h($customer->full_name()); ?></p>

                <form action="<?php echo url_for('/customers/delete.php?id=' . h(u($id))); ?>" method="post">
                  <div id="operations" class="btn-group">
                    <input type="submit" name="commit" class="btn btn-danger border-0" value="Yes" />
                    <a href="<?php echo url_for('/customers/index.php'); ?>" class="btn btn-info">No</a>
                  </div>
                </form>
              </div>
            </div>
