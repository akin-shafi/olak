<?php
class Payroll extends DatabaseObjectHR
{
  ...
  public static function find_by_employee_id($employee_id, $options = [])
  {
    ...
    if ($month) :
      $sql .= " AND month LIKE'%" . self::$database_hr->escape_string($month) . "%'";
    endif;
  ...
  public static function find_by_month($options = [])
  {
    $month = $options['month'] ?? false;
    ...
    $sql .= " WHERE (deleted IS NULL OR deleted = 0 OR deleted = '') ";
    $sql .= " AND month LIKE'%" . self::$database_hr->escape_string($month) . "%'";
    // $sql .= " AND payment_status='" . self::$database_hr->escape_string($payment_status) . "'";
  ...
  public static function find_by_salary_payable($options = [])
  {
    ...
    if ($lastMonth) {
      $sql .= " WHERE created_at LIKE'%" . self::$database_hr->escape_string($lastMonth) . "%'";
    }
    if ($payment_status) {
      $sql .= " AND payment_status='" . self::$database_hr->escape_string($payment_status) . "'";
    }
