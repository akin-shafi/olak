...
<?php
class PayrollItem extends DatabaseObjectHR
{
    ...
    public static function find_by_category($category)
    {
        ...
        $sql .= "WHERE category='" . self::$database_hr->escape_string($category) . "'";
    ...
    public static function find_all_payroll($option = [])
    {
        ...
        if ($category) {
            $sql .= " AND category='" . self::$database_hr->escape_string($category) . "'";
        }
    ...
    {
        $sql = "SELECT * FROM " . static::$table_name . " ";
        $sql .= "WHERE item LIKE'%" . self::$database_hr->escape_string($item) . "%'";
        $sql .= " AND (deleted IS NULL OR deleted = 0 OR deleted = '') ";
