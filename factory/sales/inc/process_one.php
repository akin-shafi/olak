...
if (is_post_request()) {
  ...
  if (isset($_POST['delete_stock'])) {
    ...
    $stocks = StockPhaseOne::find_by_category_id($stockId);

    foreach ($stocks as  $value) {
      $stock = StockPhaseOne::find_by_id($value->id);
      ...
      <thead>
        ...
        <tr class="text-center border">
          <?php foreach ($fullGroup as $data) :
            ...
            $color = str_merger($categoryName) ?? $categoryName;
          ?>
            <th style="border:1px solid <?php echo $color ?>;font-size: 12px;font-weight:bold">
