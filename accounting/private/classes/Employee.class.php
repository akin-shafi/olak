<?php
class Employee extends DatabaseObjectHR
{
  ...
  public static function find_by_company_name($name, $options = [])
  {
    ...
    elseif ($companyName == 'not set') :
      ...
      $sql .= "WHERE company='" . self::$database_hr->escape_string($name) . "'";
    endif;
  ...
  public static function find_by_query($options = [])
  {
    ...
    if (isset($id) || isset($name) || isset($designate)) {
      $sql .= "WHERE employee_id='" . self::$database_hr->escape_string($id) . "'";
      if (!empty($designate)) {
        $sql .= " OR designation_id='" . self::$database_hr->escape_string($designate) . "'";
      }
      if (!empty($name)) {
        $sql .= " OR first_name LIKE '%" . self::$database_hr->escape_string($name) . "%'";
        $sql .= " OR last_name LIKE '%" . self::$database_hr->escape_string($name) . "%'";
      }
      $sql .= " AND (deleted IS NULL OR deleted = 0 OR deleted = '') ";
    }
  ...
  {
    $sql = "SELECT * FROM " . static::$table_name . " ";
    $sql .= "WHERE gender='" . self::$database_hr->escape_string($gender) . "'";
    $sql .= " AND (deleted IS NULL OR deleted = 0 OR deleted = '') ";
    $obj_array = static::find_by_sql($sql);
    return $obj_array;
  }
