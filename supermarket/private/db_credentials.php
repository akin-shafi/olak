<?php

// Keep database credentials in a separate file
// 1. Easy to exclude this file from source code managers
// 2. Unique credentials on development and production servers
// 3. Unique credentials if working with multiple developers

if ($_SERVER['SERVER_NAME'] == 'localhost' || $_SERVER['SERVER_NAME'] == '192.168.64.2') {
    define("DB_SERVER", "localhost");
    define("DB_USER", "root");
    define("DB_PASS", "");
    define("DB_NAME", "olak_supermarket");

    define("DB_SERVER2", "localhost");
    define("DB_USER2", "root");
    define("DB_PASS2", "");
    define("DB_NAME2", "olak_license");
}else{
    define("DB_SERVER", "127.0.0.1");
    define("DB_USER", "hoteliap_ambiance_user");
	define("DB_PASS", "Akinshafi@91");
    define("DB_NAME", "hoteliap_supermarket");

    define("DB_SERVER2", "127.0.0.1");
    define("DB_USER2", "hoteliap_ambiance_user");
	define("DB_PASS2", "Akinshafi@91");
    define("DB_NAME2", "hoteliap_lincense");
}





?>