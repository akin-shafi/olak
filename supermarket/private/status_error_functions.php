...
function require_login()
{
    ...
    $subscription = Activation::find_by_product($company->product);
    $code = Activation::encrypt_decrypt($subscription->activation_code, 'decrypt');

    // print_r($code);
    $start_date  = strtotime($today);
    $end_date = strtotime($code);
    $datediff = $start_date - $end_date;

    if ($today >= $code) {
      redirect_to(url_for('pricing/'));
    }
