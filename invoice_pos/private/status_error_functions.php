...
function display_errors($errors=array()) {
  $output = '';
  if(!empty($errors)) {
    // $output .= "<div class=\"errors wow flash infinite text-light animated\">";
...
function require_login_user() {
  global $session;
  if(!$session->is_logged_in()) {
    redirect_to(url_for('/inventory/login.php'));
  } elseif ($session->is_logged_in() && empty($session->user_id)) {
    redirect_to(url_for('/inventory/dasboard.php')); //to redirects other users from riders pages
  }
}
