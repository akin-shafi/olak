...
//     if ($result === true) {
//       $new_id = $billing->id;
//       if (empty(trim($billing->invoiceNum))) {

//         $value['invoiceNum'] = "1" . str_pad($billing->id, 3, "0", STR_PAD_LEFT) . $randomNum;

//         $billing->merge_attributes($value); //to merge the waybill with the object which will be used in the waybill_format
//         // $tax['tax'] = $billing->total_amount / 100 * 5;

//         // $billing->merge_attributes($tax);
//         // This is for storing waybill number&format and route format to the created transaction
...
//             // "billing_format" => $args['billing_format'],
...
//           // echo '<pre>'; print_r($dataDesc); echo '</pre>';

//           $result = $expRequest->save();

//           $billing = new Billing($args);
//           // echo '<pre>'; print_r($billing); echo '</pre>';
...
//       // Logfile
//       // log_action('New Request', "id: {$expRequest->id}, Created by {$expRequest->requester}", "Request");

//       $session->message('Invoice generated successfully.');
//       // redirect_to(url_for('/chart/draft.php?invoiceNum='.$exRequest->invoiceNum));
//       redirect_to(url_for('invoice/all_invoices.php'));
//     } else {
//       // show errors
//       $expRequest = new Invoice;
//     }
//   }
// } else {
//   // display the form
