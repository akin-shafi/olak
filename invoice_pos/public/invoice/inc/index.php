...
if (is_post_request()) {
	...
	if (isset($_POST['new_invoice'])) {
		...
		$duplicate = Billing::find_duplicate(['client_id' => $client_id, 'grand_total' => $grand_total, 'created_by' => $created_by, 'created_date' => $args['created_date']]);
		if(!empty($duplicate)){
			exit(json_encode(['success' => false, 'msg' => "Duplicate Transaction Please Check Receipt Number". $duplicate->invoiceNum]));
		}else{
	...
	if (isset($_POST['edit_invoice'])) {
		...
		if ($result == true) {
			...
			$unit_cost    = $_POST['unit_cost'];
			$amount       = $_POST['amount'];
				$invoice = Invoice::find_all_by_transid($invoice_no);
				for ($i = 0; $i < count($amount); $i++) {
					$dataDesc = [
						"transid" 			  => $invoice_no,
						"service_type"  	  => $service_type[$i],
