...
if (is_get_request()) {
  if (isset($_GET['get_note'])) {
    $salaryDetails = SalaryAdvanceDetail::find_by_id($_GET['loan_id']);
    $note = $salaryDetails->note ? $salaryDetails->note : '<i>Not set</i>';
    http_response_code(200);
    $response['note'] = $note;
  }

  if (isset($_GET['deleted'])) {
    SalaryAdvanceDetail::deleted($_GET['loan_id']);
    http_response_code(200);
    $response['message'] = 'Record deleted successfully';
  }

  if (isset($_GET['get_long_note'])) {
    $longDetails = LongTermLoanDetail::find_by_id($_GET['loan_id']);
    $note = $longDetails->note ? $longDetails->note : '<i>Not set</i>';
    http_response_code(200);
    $response['note'] = $note;
  }

  if (isset($_GET['deleteLong'])) {
    LongTermLoanDetail::deleted($_GET['loan_id']);
    http_response_code(200);
    $response['message'] = 'Record deleted successfully';
  }
}
